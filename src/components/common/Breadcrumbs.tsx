...
export const Breadcrumbs: React.FC<BreadcrumbsProps> = ({ items, className = '' }) => {
  return (
    <nav className={`flex items-center gap-1.5 text-xs text-slate-500 overflow-x-auto py-1 ${className}`}>
      <Link to="/" className="text-slate-400 hover:text-teal-700 transition flex items-center">
        ...
        return (
          <React.Fragment key={index}>
            ...
            {item.href && !isLast ? (
              <Link
                to={item.href}
                className="hover:text-teal-700 transition font-medium whitespace-nowrap"
              >
                {item.label}
              </Link>
            ) : (
